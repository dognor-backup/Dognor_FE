...
export default function AddPatInfoCard() {
  ...
  const bloodTypeList = [
    ...
    "DEA 2-", "DEA 2+", "DEA 3-", "DEA 3+", 
    "DEA 4-", "DEA 4+", "DEA 5-", "DEA 5+", 
    "DEA 6-", "DEA 6+", "DEA 7-", "DEA 7+", 
    "DEA 8-", "DEA 8+", "DEA 9-", "DEA 9+", 
    "DEA 10-", "DEA 10+", "DEA 11-", "DEA 11+", 
    "DEA 12-", "DEA 12+", "DEA 13-", "DEA 13+", 
    "기타/모름"
  ];
  ...
  const handleSavePatInfo = (e) => {
    ...
    const requiredFields = ['name', 'birthday', 'breedCd', 'bloodTypeCd', 'isDonor', 'imgFile', 'userSeq'];
    const missingFields = requiredFields.filter(field => !updatedInputValues[field]);
    ...
    mutation.mutate(updatedInputValues, {
      onSuccess: (response) => {
        if (response.success) {
          ...
          alert(response.msg || '반려견 정보 추가에 실패했습니다.');
        }
      },
      onError: () => {
        alert('반려견 정보 추가 중 오류가 발생했습니다.');
      }
    });
  };
