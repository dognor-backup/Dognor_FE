...
export const updateDonationStory = async (data) => {
  try {
    const formData = new FormData();
    
    formData.append("donationStorySeq", data.donationStorySeq);
    formData.append("content", data.content);
    ...
    const response = await AxiosFormDataInstance.put("/donation-story", formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      },
      timeout: 10000
...
export const deleteDonationStory = async (donationStorySeq) => {
  try {
    const response = await AxiosInstance.delete(`/donation-story/${donationStorySeq}`);
    ...
    return { 
      success: false, 
      msg: response.data.msg || "헌혈 이야기 삭제에 실패했습니다."
    };
  } catch (error) {
    if (error.response) {
      const { status, data } = error.response;
